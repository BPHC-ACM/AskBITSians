...
export default function ChatRequests({ userId }) {
  ...
  const fetchPastRequests = async () => {
    ...
    try {
      const response = await fetch(
        `/api/chat-requests/past?alumnus_id=${userId}`
      );
  ...
  useEffect(() => {
    const fetchChatRequests = async () => {
      try {
        const response = await fetch(`/api/chat-requests?alumnus_id=${userId}`);
        const data = await response.json();
            ...
            {pastRequests.length > 0 ? (
              <>
                ...
                <div className={styles.paginationContainer}>
                  <Pagination
                    count={Math.ceil(pastRequests.length / itemsPerPage)}
                    page={page}
              ...
              <p className={styles.noRequests}>No past requests found.</p>
            )}
          </div>
        </div>
      )}
