...
export default function ChatsMain({ selectedRoom, userId, setSelectedRoom }) {
  ...
  return (
    <div
      className={`chat-display ${selectedRoom && isMobileView ? 'active' : ''}`}
    >
      <AnimatePresence mode='wait'>
        {selectedRoom ? (
          ...
          >
            <div className={`header ${selectedRoom ? 'active' : ''}`}>
              <img
                src={`/api/avatar?name=${encodeURIComponent(userName || '')}`}
                alt='User Avatar'
                className='avatar'
              />
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className='timestamp'
                      style={{
                        left: isUser ? '0.5rem' : 'auto',
                        right: isUser ? 'auto' : '0.5rem',
                      }}
                    >
            ...
            <div className='message-input-container'>
              ...
              />
              <IconSend size={25} className='send-icon' onClick={sendMessage} />
            </div>
          </motion.div>
        ) : (
          <motion.div
